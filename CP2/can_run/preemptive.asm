...
00120$:
	mov	_currID,r6
;	preemptive.c:275: if( currID == 0 )nextID = 1;
	mov	a,_currID
	jnz	00110$
	mov	_nextID,#0x01
	sjmp	00111$
00110$:
;	preemptive.c:276: else if( currID == 1 )nextID = 2;
	mov	a,#0x01
	cjne	a,_currID,00107$
	mov	_nextID,#0x02
	sjmp	00111$
00107$:
;	preemptive.c:277: else if( currID == 2 )nextID = 4;
	mov	a,#0x02
	cjne	a,_currID,00104$
	mov	_nextID,#0x04
	sjmp	00111$
00104$:
;	preemptive.c:278: else if( currID == 3 )nextID = 8;
	mov	a,#0x03
	cjne	a,_currID,00111$
	mov	_nextID,#0x08
00111$:
;	preemptive.c:279: if( bitmap & nextID ){
	mov	a,_nextID
	anl	a,_bitmap
	jz	00114$
;	preemptive.c:301: __endasm;  
	POP	ACC
	MOV	R7, A
	POP	ACC
	MOV	R6, A
...
;	preemptive.c:303: RESTORESTATE;
	...
	POP B 
	POP ACC 
;	preemptive.c:304: EA = 1;
;	assignBit
	setb	_EA
;	preemptive.c:307: __endasm;
	RETI
;	preemptive.c:308: }
	ret
;------------------------------------------------------------
...
;------------------------------------------------------------
;	preemptive.c:315: void ThreadExit(void)
;	-----------------------------------------
;	 function ThreadExit
;	-----------------------------------------
_ThreadExit:
